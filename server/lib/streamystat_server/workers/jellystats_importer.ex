defmodule StreamystatServer.Workers.JellystatsImporter do
  ...
  defp process_items_batch(server, items) when is_list(items) do
    ...
    try do
      ...
      libraries_map = Repo.transaction(fn ->
      ...
      libraries_map = case libraries_map do
        {:ok, map} -> map
        _ -> %{}
      end

      # Collect all existing jellyfin_ids - in its own transaction
      existing_map = Repo.transaction(fn ->
      ...
      existing_map = case existing_map do
        {:ok, map} -> map
        _ -> %{}
      end
  ...
  defp process_playback_batch(server, activities, users_map, items_map) do
    ...
    existing_sessions = if Enum.empty?(item_ids) || Enum.empty?(user_ids) do
      %{}
    else
      ...
      try do
        ...
        case query_result do
          {:ok, existing_sessions_list} ->
            ...
            Enum.reduce(existing_sessions_list, %{}, fn {item_id, user_id, start_time, session}, acc ->
    ...
    Enum.reduce(activities, %{created: 0, updated: 0, skipped: 0, errors: 0}, fn activity, acc ->
      try do
        result = Repo.transaction(fn ->
          create_playback_session_from_jellystats(server, activity, users_map, items_map, existing_sessions)
        end, timeout: @db_timeout)
