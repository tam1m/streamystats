defmodule StreamystatServerWeb.BackupController do
  ...
  def export(conn, %{"server_id" => server_id_str}) do
    ...
    {:ok, stmt} = Exqlite.Sqlite3.prepare(db, "INSERT INTO playback_sessions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
    ...
    Repo.transaction(fn ->
      ...
      |> Stream.each(fn s ->
        ...
        end_time_str = if s.end_time, do: s.end_time |> DateTime.to_naive() |> NaiveDateTime.to_iso8601(), else: nil
    ...
    try do
      ...
      entries =
        ...
        |> Stream.map(fn row ->
          ...
          |> Enum.zip(row)
          |> Map.new()
          |> Map.put(:server_id, server_id) # Ensure the correct server_id is used
          |> format_attrs()
        end)
        |> Enum.to_list()
  ...
  defp parse_bool(_),  do: nil # Default to nil if unsure
  ...
  defp format_attrs(attrs) do
    ...
    %{
      ...
      start_time:        parse_utc(Map.get(attrs, :start_time)), # Expects DateTime or nil
      end_time:          parse_utc(Map.get(attrs, :end_time)), # Expects DateTime or nil
      ...
      server_id:         Map.get(attrs, :server_id), # Already set in process_import, but Map.get won't hurt
      # Generate new timestamps for the import, truncated to seconds
      inserted_at:       NaiveDateTime.utc_now() |> NaiveDateTime.truncate(:second),
      updated_at:        NaiveDateTime.utc_now() |> NaiveDateTime.truncate(:second)
    }
    |> Map.drop([:id]) # Drop the old ID from the backup file
  end
  ...
  defp parse_utc(iso8601_string) when is_binary(iso8601_string) do
    with {:ok, naive_dt} <- NaiveDateTime.from_iso8601(iso8601_string),
      ...
      datetime # Return the DateTime struct directly
    else
  ...
  defp parse_utc(_other), do: nil # Handle non-string inputs gracefully
end
