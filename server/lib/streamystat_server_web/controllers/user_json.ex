defmodule StreamystatServerWeb.UserJSON do
  ...
  def show(%{user: user, watch_history: watch_history, watch_stats: watch_stats}) do
    %{data: data(%{user: user, watch_stats: watch_stats}, :show, watch_history)}
  end
  ...
  defp data(%{user: %User{} = user, watch_stats: watch_stats}, :show, watch_history) do
    %{
      id: user.id,
      jellyfin_id: user.jellyfin_id,
      name: user.name,
